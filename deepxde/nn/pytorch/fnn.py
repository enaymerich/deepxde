...
class FNNaux(NN):
    """Fully-connected neural network, with auxiliary vars for parametric learning""" 
    ...
    def forward(self, inputs):
        x = torch.hstack((inputs,self.auxiliary_vars))
        if self._input_transform is not None:
            x = self._input_transform(x)
        for j, linear in enumerate(self.linears[:-1]):
            x = (
        ...
        x = self.linears[-1](x)
        if self._output_transform is not None:
            x = self._output_transform(inputs, self.auxiliary_vars, x)
        return x
