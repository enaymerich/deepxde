...
def net_output_HF(xy_data,t):
    ...
    test_predict_HP = my_nn(test_input) * (max_temp - min_temp) + min_temp
    test_predict_HF = -K/Z*dde.grad.jacobian(test_predict_HP, test_input, i=0, j=0)
...
checkpoint = torch.load('../../normtile_fullopt/models/SKOPT_normtile_BC_weak_97_neurons_10_layers_9.4e-05_lr.pt')
#checkpoint = torch.load('./SKOPT_normtile_BC_weak_32_neurons_2_layers_3.0e-04_lr.pt')
...
f = r"../../animations/animation_0.75_norm_net.gif"
...
f = r"../../animations/animation_0.75_norm_net_heatflux.gif"
writergif = animation.PillowWriter(fps=15)
anim.save(f, writer=writergif)
...
Tvec = create_temp(y_tile, A=max_temp-min_temp, sigma=sigma*width, mu=width/2, T0=min_temp)
...
f = r"../../animations/animation_0.75_theo_heatflux.gif"
writergif = animation.PillowWriter(fps=15)
anim.save(f, writer=writergif)
...
f = r"../../animations/animation_0.75_norm_error.gif"
writergif = animation.PillowWriter(fps=15)
anim.save(f, writer=writergif)
...
f = r"../../animations/animation_0.75_error_heatflux.gif"
writergif = animation.PillowWriter(fps=15)
anim.save(f, writer=writergif)